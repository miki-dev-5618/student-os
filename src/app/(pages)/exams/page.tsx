    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 text-neutral-800 dark:text-neutral-100">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">Track and manage all your academic exams.</p>
        </div>
        <button
      ...
      {formOpen && (
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-xl shadow-md space-y-4">
          ...
          <form action={createExamAction} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
      ...
      <div className="overflow-x-auto bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-sm">
        <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-850">
          <thead className="bg-neutral-50 dark:bg-neutral-800/50">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-bold text-neutral-500 uppercase tracking-wider">Exam</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-neutral-500 uppercase tracking-wider">Subject</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-neutral-500 uppercase tracking-wider">Exam Date</th>
          ...
          <tbody className="divide-y divide-neutral-150 dark:divide-neutral-850">
            {exams.length === 0 ? (
              ...
              exams.map((exam: any) => (
                <tr key={exam.examId} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/30 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">{exam.title}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-550">{exam.subjectName}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-550">{new Date(exam.examDate).toLocaleDateString()}</td>
