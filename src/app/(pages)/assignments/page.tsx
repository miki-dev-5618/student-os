    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 text-neutral-800 dark:text-neutral-100">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">Track and manage all your academic assignments.</p>
        </div>
        <button
      ...
      {formOpen && (
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-xl shadow-md space-y-4">
          ...
          <form action={createAssignmentAction} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-neutral-400">Deadline</label>
              <input
                type="date"
                name="deadline"
                required
      ...
      <div className="overflow-x-auto bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-sm">
        <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-850">
          <thead className="bg-neutral-50 dark:bg-neutral-800/50">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-bold text-neutral-500 uppercase tracking-wider">Assignment</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-neutral-500 uppercase tracking-wider">Subject</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-neutral-500 uppercase tracking-wider">Due Date</th>
          ...
          <tbody className="divide-y divide-neutral-150 dark:divide-neutral-850">
            {assignments.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-sm text-neutral-550">
                  No assignments found. Click "Add Assignment +" to create one.
                </td>
              </tr>
            ) : (
              assignments.map((assignment: any) => (
                <tr key={assignment.assignmentId} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/30 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">{assignment.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-500">{assignment.subjectName}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-550">{new Date(assignment.deadline).toLocaleDateString()}</td>
